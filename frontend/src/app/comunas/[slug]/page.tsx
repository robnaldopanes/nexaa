...
export default function ComunaPage({ params }: { params: { slug: string } }) {
  ...
  const name = comuna?.name || params.slug.replace(/-/g, ' ');

  const [newsList, setNewsList] = useState<NewsItem[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function loadComunaNews() {
      try {
        setLoading(true);
      ...
      <main className="pt-14 pb-20 overflow-x-hidden">
        <section className="px-margin-mobile mt-stack-md">
          ...
          <p className="text-body-md text-on-surface-variant mb-4">
            Noticias de {name}, Región de Ñuble
          </p>
        </section>
        ...
        <section className="px-margin-mobile space-y-gutter">
          {loading ? (
            <div className="text-center py-16">
              ...
              <p className="text-body-lg text-on-surface-variant mt-3">Buscando noticias de {name}...</p>
            </div>
          ) : newsList.length === 0 ? (
            <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-xl p-8 text-center mt-4">
              <span className="material-symbols-outlined text-on-surface-variant text-[64px]">location_off</span>
              <p className="text-body-lg text-on-surface-variant mt-3">No hay noticias en {name} aún</p>
              <p className="text-label-md text-on-surface-variant/60 mt-1">Las noticias locales aparecerán aquí una vez que sean aprobadas por el administrador.</p>
            </div>
          ) : (
            newsList.map((news) => (
              <NewsCard
